...
namespace NCDK.Isomorphisms
{
    ...
    public sealed class Mappings : IEnumerable<int[]>
    {
        ...
        internal
#endif
        Mappings(IAtomContainer query, IAtomContainer target, IEnumerable<int[]> iterable)
        {
            this.query = query;
            this.target = target;
        ...
        public IEnumerable<IAtomContainer> ToSubstructures()
        {
            return GetMapping(new AtomBondMaper(query, target)).Select(map =>
            {
                ...
                foreach (var atom in query.Atoms)
                    submol.Add((IAtom)map[atom]);
                foreach (var bond in query.Bonds)
                    submol.Add((IBond)map[bond]);
                return submol;
            });
        }
		 ...
		 * manually.
		 *
		 * The method is simply invokes <pre>mappings.GetUniqueAtoms().Count()</pre>.
		 *
		 * @return number of matches
		 */
