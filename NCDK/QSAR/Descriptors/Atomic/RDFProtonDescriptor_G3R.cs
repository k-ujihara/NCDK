...
namespace NCDK.QSAR.Descriptors.Atomic
{
    ...
    public class RDFProtonDescriptor_G3R : AbstractAtomicDescriptor, IAtomicDescriptor
    {
        ...
        /// <para>Parameters are the proton position and a bool (true if you need to detect aromaticity)</para>
        /// </summary>
        ...
        public DescriptorValue Calculate(IAtom atom, IAtomContainer atomContainer, IRingSet precalculatedringset)
        {
            ...
            // DURING DETECTION
            List<int> singles = new List<int>(); // list of any bond not
                                                 // rotatable
            List<int> doubles = new List<int>(); // list with only double bonds
            List<int> atoms = new List<int>(); // list with all the atoms in
                                               // spheres
                                               // atoms.Add( int.ValueOf( mol.Atoms.IndexOf(neighboors[0]) ) );
            List<int> bondsInCycloex = new List<int>(); // list for bonds in
            ...
            bool theBondIsInA6MemberedRing; // this is like a flag for bonds
                ...
                if (mol.Atoms.IndexOf(curAtomSecond) != atomPosition && GetIfBondIsNotRotatable(mol, secondBond, detected))
                {
                    ...
                    foreach (var curAtomThird in atomsInThirdSphere)
                    {
                                ...
                                && GetIfBondIsNotRotatable(mol, thirdBond, detected))
                        {
                            ...
                            // the bool "theBondIsInA6MemberedRing" is set to
                            // true
        ...
        private int GetNearestBondtoAGivenAtom(IAtomContainer mol, IAtom atom, IBond bond)
        {
            int nearestBond = -1;
            double distance = 0;
            var bondsAtLeft = mol.GetConnectedBonds(bond.Atoms[0]);
