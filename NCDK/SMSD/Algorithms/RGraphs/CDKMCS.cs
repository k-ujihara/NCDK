    ...
    public static class CDKMCS
    {
        ...
        public static bool IsIsomorph(IAtomContainer sourceGraph, IAtomContainer targetGraph, bool shouldMatchBonds)
        {
            ...
            if (targetGraph.Atoms.Count == 1)
            {
                ...
                {
                    return qAtom2.Matches(sourceGraph.Atoms[0]);
                }
                else
                {
        ...
        public static IReadOnlyList<CDKRMap> GetIsomorphMap(IAtomContainer sourceGraph, IAtomContainer targetGraph, bool shouldMatchBonds)
        {
            if (sourceGraph is IQueryAtomContainer)
            {
                throw new CDKException("The first IAtomContainer must not be an IQueryAtomContainer");
        ...
        public static IReadOnlyList<CDKRMap> GetIsomorphAtomsMap(IAtomContainer sourceGraph, IAtomContainer targetGraph, bool shouldMatchBonds)
        {
            if (sourceGraph is IQueryAtomContainer)
            {
                throw new CDKException("The first IAtomContainer must not be an IQueryAtomContainer");
        ...
        /// <exception cref="CDKException"></exception>
        public static IReadOnlyList<IReadOnlyList<CDKRMap>> GetIsomorphMaps(IAtomContainer sourceGraph, IAtomContainer targetGraph, bool shouldMatchBonds)
        {
        ...
        /// <exception cref="CDKException"></exception>
        public static IReadOnlyList<IReadOnlyList<CDKRMap>> GetSubgraphMaps(IAtomContainer sourceGraph, IAtomContainer targetGraph, bool shouldMatchBonds)
        {
                ...
                bool shouldMatchBonds)
        {
            ...
            if (list == null)
            {
                return MakeAtomsMapsOfBondsMaps(GetSubgraphMaps(sourceGraph, targetGraph, shouldMatchBonds), sourceGraph, targetGraph);
            }
            else
            {
                var atomsMap = new List<IReadOnlyList<CDKRMap>>
                {
                    list
                };
                return atomsMap;
        ...
        /// <exception cref="CDKException"></exception>
        public static IReadOnlyList<CDKRMap> GetSubgraphAtomsMap(IAtomContainer sourceGraph, IAtomContainer targetGraph, bool shouldMatchBonds)
        {
            var list = CheckSingleAtomCases(sourceGraph, targetGraph);
            if (list == null)
            {
        ...
        /// <exception cref="CDKException"></exception>
        public static IReadOnlyList<IAtomContainer> GetOverlaps(IAtomContainer sourceGraph, IAtomContainer targetGraph, bool shouldMatchBonds)
        {
            var rMapsList = Search(sourceGraph, targetGraph, new BitArray(sourceGraph.Bonds.Count), new BitArray(targetGraph.Bonds.Count), true, false, shouldMatchBonds);

            // projection on G1
            var graphList = ProjectList(rMapsList, sourceGraph, IdType.Id1);
        ...
        /// <exception cref="CDKException"></exception>
        public static IReadOnlyList<IReadOnlyList<CDKRMap>> Search(IAtomContainer sourceGraph, IAtomContainer targetGraph,
        ...
        private static IAtomContainer Project(IReadOnlyList<CDKRMap> rMapList, IAtomContainer graph, IdType key)
        {
            ...
            foreach (var rMap in rMapList)
            {
                ...
                atom = bond.Atoms[0];
                if (!table.TryGetValue(atom, out IAtom atom1))
                {
                ...
                atom = bond.Atoms[1];
                if (!table.TryGetValue(atom, out IAtom atom2))
                {
        ...
        private static IReadOnlyList<IAtomContainer> ProjectList(IReadOnlyList<IReadOnlyList<CDKRMap>> rMapsList, IAtomContainer graph, IdType key)
        {
        ...
        private static IReadOnlyList<IAtomContainer> GetMaximum(IReadOnlyList<IAtomContainer> graphList, bool shouldMatchBonds)
        {
            var reducedGraphList = new List<IAtomContainer>(graphList);
