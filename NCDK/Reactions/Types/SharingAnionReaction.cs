...
namespace NCDK.Reactions.Types
{
    ...
    public partial class SharingAnionReaction : AbstractSharingReaction
    {
        ...
        public override IReactionSet Initiate(IAtomContainerSet<IAtomContainer> reactants, IAtomContainerSet<IAtomContainer> agents)
        {
            return base.Initiate(reactants, agents,
                ...
                (mol, atom) => atom.FormalCharge == -1
                        && !mol.GetConnectedSingleElectrons(atom).Any()
                        && mol.GetConnectedLonePairs(atom).Any(),
                atom => atom.FormalCharge == 1,
                bond => bond.Order == BondOrder.Single);
        }
    }
}
