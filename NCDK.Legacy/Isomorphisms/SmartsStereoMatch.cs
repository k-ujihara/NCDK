...
namespace NCDK.Isomorphisms
{
    ...
    public sealed class SmartsStereoMatch
    {
        ...
        private bool CheckTetrahedral(int u, int[] mapping)
        {
            ...
            var queryAtom = (SMARTSAtom)query.Atoms[u];
            var targetAtom = target.Atoms[v];

            var us = Neighbors(queryElement, queryMap);
            us = Map(u, v, us, mapping);
            var p = PermutationParity(us);
            ...
            var q = PermutationParity(vs) * Parity(targetElement.Stereo);
        ...
        private bool CheckGeometric(int u1, int u2, int[] mapping)
        {
            ...
            var p = Parity(queryElement.Stereo);
            var q = Parity(targetElement.Stereo);

            var uLeft = queryMap[queryBonds[0].GetOther(query.Atoms[u1])];
            var uRight = queryMap[queryBonds[1].GetOther(query.Atoms[u2])];

            var vLeft = targetMap[targetBonds[0].GetOther(target.Atoms[v1])];
            var vRight = targetMap[targetBonds[1].GetOther(target.Atoms[v2])];
