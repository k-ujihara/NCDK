...
using static NCDK.Graphs.JumboPathGraph;
...
namespace NCDK.Graphs
{
    ...
    public class JumboPathGraphTest
    {
        ...
        public virtual void RepeatRemoval()
        {
            ...
            var pg = new JumboPathGraph(k3, Identity(3), ord);
        ...
        public virtual void K8()
        {
            ...
            var pg = new JumboPathGraph(k8, Identity(8), ord);
            List<int[]> cycles = new List<int[]>();
            for (int v = 0; v < ord; v++)
                pg.Remove(v, cycles);
        ...
        {
            Assert.IsFalse(new SimpleEdge(0, 1, 8).IsLoop);
            Assert.IsTrue(new SimpleEdge(0, 0, 8).IsLoop);
            Assert.IsFalse(new ReducedEdge(new SimpleEdge(0, 1, 8), new SimpleEdge(2, 1, 8), 1).IsLoop);
            Assert.IsTrue(new ReducedEdge(new SimpleEdge(0, 1, 8), new SimpleEdge(0, 1, 8), 0).IsLoop);
            Assert.IsTrue(new ReducedEdge(new SimpleEdge(0, 1, 8), new SimpleEdge(0, 1, 8), 1).IsLoop);
        }
        ...
        public virtual void Path()
        {
            Assert.IsTrue(Compares.AreDeepEqual(
                new int[] { 0, 1 },
                new SimpleEdge(0, 1, 8).Path()));
            Assert.IsTrue(Compares.AreDeepEqual(
                new int[] { 0, 1, 2 },
                new ReducedEdge(new SimpleEdge(0, 1, 8), new SimpleEdge(2, 1, 8), 1).Path()));
            Assert.IsTrue(Compares.AreDeepEqual(
                new int[] { 0, 1, 2, 3, 4 },
                new ReducedEdge(new ReducedEdge(new SimpleEdge(0, 1, 8), new SimpleEdge(2, 1, 8), 1), new ReducedEdge(
                    new SimpleEdge(2, 3, 8), new SimpleEdge(3, 4, 8), 3), 2).Path()));
        }

        [TestMethod()]
        public virtual void Disjoint()
        {
            var e = new ReducedEdge(new SimpleEdge(0, 1, 8), new SimpleEdge(2, 1, 8), 1);
            var f = new ReducedEdge(new SimpleEdge(2, 3, 8), new SimpleEdge(3, 4, 8), 3);
        ...
        {
            Assert.AreEqual(2, new SimpleEdge(0, 1, 8).Length);
            var e = new ReducedEdge(new SimpleEdge(0, 1, 8), new SimpleEdge(2, 1, 8), 1);
            var f = new ReducedEdge(new SimpleEdge(2, 3, 8), new SimpleEdge(3, 4, 8), 3);
