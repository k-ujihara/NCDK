...
namespace NCDK.IO
{
    ...
    public class MDLV3000WriterTest
    {
        ...
        {
            using (var mdlr = new MDLV2000Reader(ResourceLoader.GetAsStream("NCDK.Data.MDL.triphenyl-phosphate-expanded.mol")))
            {
                var mol = mdlr.Read(builder.NewAtomContainer());
                var res = WriteToStr(mol);
        ...
        {
            using (var mdlr = new MDLV2000Reader(ResourceLoader.GetAsStream("NCDK.Data.MDL.sgroup-ord-mixture.mol")))
            {
                var mol = mdlr.Read(builder.NewAtomContainer());
                var res = WriteToStr(mol);
                Assert.IsTrue(res.Contains(
